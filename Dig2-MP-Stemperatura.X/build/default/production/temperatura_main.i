...
# 2 "<built-in>" 2
# 1 "temperatura_main.c" 2
# 11 "temperatura_main.c"
...
uint8_t map (char valor);

void semaforo (uint8_t temperatura){
    if (temperatura < 24){
        RD2 = 0;
        RD3 = 0;
        RD7 = 1;
    ...
    else if (temperatura >= 24 && temperatura <=27){
        RD2 = 0;
        RD3 = 1;
        RD7 = 0;
    }
...
# 100 "temperatura_main.c"
void __attribute__((picinterrupt(("")))) isr(void){
    if (PIR1bits.SSPIF){
        trash = spiRead();
        spiWrite(grados.mapeo);
        PIR1bits.SSPIF = 0;
    }
...
void main(void) {
    ...
    while (1){
        ...
        grados.mapeo = map(grados.input);
        semaforo (grados.mapeo);
    }
}
